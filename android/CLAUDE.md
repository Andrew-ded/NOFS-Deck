...
## Ветка feature/core — ЯДРО ПРОДУКТА (главное, читать первым)
После «наделали фич — не складывается в продукт» (ветка feature/carve) отрезано
всё, кроме ядра: **метрики ПК + макросы + плеер**, плюс скринсейвер, телефонная
раскладка и настройки подключения. Рез выполнен по принципу «убрать из склейки»:
git, микшер, плейтайм, паспорт файла, сцена сборки, QR, remote typing, игровой
режим НЕ рендерятся и НЕ грузятся, но их файлы лежат на месте (components/,
Services/) — вернуть можно точечно. Где резалось: DeskScreen.kt (правый слот
теперь только плеер), PhoneDeskScreen.kt (плеер оверлеем, без git),
HeaderAndMiniPlayer.kt (кнопка игрового режима удалена), AgentHost.cs (только
metrics/media/context/macros + macroState), Program.cs (ExplorerMenu.Unregister),
config.json/example (без builds, github, repoPath, remoteType; build:-макросы
заменены на keys:-хоткеи IDE).

Ось продукта (сформулировал пользователь): не «кнопки на стекле», а **живой
пульт с крутой обратной связью** — контрол ОТРАЖАЕТ реальное состояние ПК
(пример: кнопка «Жирный» подсвечена, когда курсор в жирном тексте, и гаснет
сама). Три слоя отклика: тактильный (вибро), контрол=статус (тумблер виден по
факту), физика нажатия. Метрики уже «дышат», плеер морфит — макросы делаем
рефлективными и тактильными.

СДЕЛАНО (рефлективные макросы, аддитивно, не проверено компилятором):
- Агент: `Services/MacroStateService.cs` — Eval(state) по надёжным источникам
  (caps/num/scroll lock через GetKeyState, мьют микро/мастера, играет ли медиа);
  `MacroConfig.State`, `MacroDto.Active`; в AgentHost цикл `PushMacroStateAsync`
  (250мс, пуш макросов только при смене сигнатуры состояний); дешёвые
  AudioService.MicMuted()/MasterMuted(), MediaService.IsPlaying(); в Keys
  добавлены capslock/numlock/scrolllock. Конфиг: рефлективные системные кнопки
  Caps/Num/Тишина/Медиа (`state`).
- Планшет: `Macro.active`; `MacroCard` — подсветка акцентом+рамка при active,
  гаптика (LocalHapticFeedback) на тап, физика (проседание с пружиной по
  collectIsPressedAsState). Демо: тап по не-launch макросу переключает active.

ДАЛЬШЕ по рефлективности: флагман — «жирный шрифт в Word» через UIAutomation
(TextPattern, атрибут IsBold) — хрупко, но это тот самый «вау». Плюс: текущее
аудио-устройство, раскладка клавиатуры, состояние Focus Assist. Потом — реальный
карв главного экрана (убрать правый слот и лишнее из DeskScreen).
...
- **Телефонный экран**: `DeskScreen()` — точка входа-диспетчер, роутит по
  ...
  GitHub подтягивается автоматически, не по свайпу как на планшете — но
  запрос живёт в `PhoneDeskScreen` (один раз на CONNECTED), НЕ в самой
  `PhoneGitPanel`: та монтируется/размонтируется при каждом открытии-
  закрытии плеера (условная композиция в `PhoneGitAndPlayerSlot`), и
  запрос внутри неё дублировался на каждый тап по плееру и рвал анимацию
  слайда лишним стейт-чёрном — было замечено как лаги/дёрганье плеера
  на телефоне, почищено), плеер — тот же `PlayerSheet`/`BottomPlayerPill`, что на
...
- **«Паспорт файла» (ф.9, только планшет)**: агент `Services/FilePassportService.cs` —
  тикает в 2с-цикле контекста, читает заголовок активного окна
  (`GetForegroundWindow`+`GetWindowText`), регексом вытаскивает ведущее имя
  файла с известным расширением (`GitService.cs — NofsAgent — Rider`), при
  смене файла ищет его в `config.RepoPath` (`Directory.EnumerateFiles`, мимо
  bin/obj/.git/node_modules/...) и грепом (НЕ Roslyn/MSBuildWorkspace —
  осознанно дешёвый вариант для первой итерации, см. issue #19: тяжёлая
  зависимость + заточен только под C#, а в фокусе может быть и Kotlin)
  вытаскивает объявленные типы/методы (regex по `class/interface/...` и
  сигнатурам методов C#/Kotlin), `using`/`import` как зависимости, и грепом
  по остальным `*.cs/*.kt/*.java/*.ts/*.tsx` — где встречается primary-тип
  файла (до 8 файлов). Протокол: `filePassport` (`fileName=""` — скрыть).
  Планшет: `DeskState.filePassport: FilePassportState`, `ui/components/
  FilePassportCard.kt` — НЕ полноэкранный оверлей (файл в IDE меняется
  каждые несколько секунд, фуллскрин на каждое переключение был бы
  невыносим), а компактная карточка снизу-слева, гаснет через 12с или по
  тапу; рендерится только в `TabletDeskScreen` и вне игрового режима.

ЗНАЮ ПРО НЕДОДЕЛКИ:
- «Паспорт файла»: детекция файла — грубая эвристика по заголовку окна, если
  IDE не кладёт имя файла первым токеном (нестандартная тема/локализация) —
  просто не сработает, молча. Грепом можно словить ложные совпадения имени
  типа в комментариях/строках (не AST). Полный скан репозитория на КАЖДУЮ
  смену файла (`FindUsages`) может тормозить на большом репозитории — для
  этого проекта и подобных hobby-масштабов ок, не проверялось на крупных
  монорепах. Демо-режим эту карточку не показывает (нечего эмулировать
  честно — фича завязана на реальный live-статус IDE на ПК). Не проверено
  компилятором и визуально — см. общее предупреждение ниже.
...
УБРАНО ИЗ БЭКЛОГА (issues, отклонены владельцем после ревью): Obsidian (ф.1,
issue #14), предиктивные алерты (ф.3, issue #15), git-дифф+«пока меня не
было» (ф.4, issue #16), second-screen логи (ф.5, issue #17 — заменена
issue #21 «Запущенный проект»).

ОСТАЛОСЬ: presence-режим (ф.6, issue #18 — открыт вопрос по источнику
календаря), «Запущенный проект» (issue #21 — плитка процесса с открытым
портом, тап открывает браузер/swagger, открыты вопросы по детекции).
GitHub issues — источник истины по статусу, этот файл может отставать.
...
app/src/main/java/com/nofs/desk/
  ...
  ui/
    ...
    components/
      ...
      FilePassportCard.kt         угловая карточка «Паспорт файла» (только планшет)
...
agent/NofsAgent/                  .NET 8 трей-агент (см. agent/README.md)
  ...
  Services/FilePassportService.cs заголовок окна -> имя файла -> греп repoPath ("Паспорт файла")
```
...
1. Схлопывание макросов в вертикальную колонку из 4 иконок при открытии плеера
   ...
   по скроллу отключено (левая зон