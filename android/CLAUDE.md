...
Панель Git свопается (таб + свайп): «Локально» и «GitHub»; прячется кнопкой »»
(справа от табов) с анимацией веса колонки; возвращается КРУГЛОЙ кнопкой рядом
с чипом подключения ПК в шапке (появляется только при спрятанной панели).
